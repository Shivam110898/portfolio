...
class BallTracker:
    ...
    def custom_ransac_filter(self):
        ...
        ransac_pre = RANSACRegressor(max_trials=500, stop_probability=0.99, min_samples=3)
        ransac_pre.fit(X_pre_bounce, y_pre_bounce)
        inliers_pre = ransac_pre.inlier_mask_

        # Apply RANSAC for post-bounce
        ransac_post = RANSACRegressor(max_trials=500, stop_probability=0.99, min_samples=2)
    ...
    def calculate_release_position(self, post_ransac_ball_detections):
        ...
        off_stump_top_x = self.image_points[2][0]  # X-coordinate of off-stump top (left)
        leg_stump_top_x = self.image_points[4][0]  # X-coordinate of leg-stump top (right)
        stump_world_width = 0.2286  # Distance between off-stump and leg-stump tops in meters

        # Calculate pixel width between off-stump and leg-stump tops (X-axis only)
        stump_pixel_width = abs(off_stump_top_x - leg_stump_top_x)
        ...
        total_release_width = stump_world_width *  release_width_in_meters

        # Return both release height and lateral width in meters
        return total_release_height, total_release_width
    ...
    def resample_trajectory(self, trajectory_3d, num_points):
        """
        Resamples the 3D trajectory to match the number of detections using interpolation.
        Args:
            trajectory_3d: Simulated 3D trajectory points (full pitch)
            num_points: Number of detection points to resample to

        Returns:
            Resampled 3D trajectory array with num_points elements.
        """
        # Interpolate based on the y-axis (depth) since it should progress continuously along the pitch
        y_coords = trajectory_3d[:, 1]
        interp_func = interp1d(y_coords, trajectory_3d, axis=0, kind='linear', fill_value="extrapolate")

        # Generate evenly spaced y values from start to end of detections
        target_y = np.linspace(y_coords[0], y_coords[-1], num_points)
        resampled_trajectory = interp_func(target_y)
    ...
    def simulate_trajectory(self, initial_velocities, t_values):
        ...
        initial_release_position = np.array([-0.35, self.release_depth, self.release_height])  # Replace with actual release height and width values
        ...
        trajectory = []
        while state[1] < 21:
            sol = solve_ivp(equations_of_motion, [0, (1/60)], state)
            state = sol.y[:, -1]
    ...
    def cost_function(self, velocity_params, detected_2d, t_values):
        ...
        re_trajectory_3d = self.resample_trajectory(trajectory_3d, len(detected_2d))

        # Project the 3D points to 2D
        projected_2d = np.array([self.project_to_image(point) for point in re_trajectory_3d])
        ...
        # Flatten the error for least-squares optimization
        error_flattened = np.sqrt(np.mean(error ** 2))

        # Debugging: Print the current error to observe optimization progress
        print(f"Velocity Params: {velocity_params}, Error Sum: {error_flattened}")
    ...
    def optimize_trajectory(self, detected_2d, t_values):
        ...
        initial_velocity_guess = np.array([-0.3, 20, 0.0])  # (vx0, vy0, vz0)
        ...
        result = minimize(objective, initial_velocity_guess, method='Powell', bounds=bounds, options={'disp': True})
...
if __name__ == "__main__":
    tracker = BallTracker("sidearm9.mp4")
    tracker.run()
