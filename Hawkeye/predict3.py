...
class BallTracker:
    ...
    def simulate_trajectory(self, initial_velocities, t_values):
        ...
        initial_release_position = np.array([-0.5, self.release_depth, self.release_height])  # Replace with actual release height and width values
        ...
        trajectory = []
        while state[1] < 21:
            sol = solve_ivp(equations_of_motion, [0, 0.016], state)
            state = sol.y[:, -1]
    ...
    def cost_function(self, velocity_params, detected_2d, t_values):
        """
        Modified cost function to prioritize alignment in earlier frames.
        Args:
        ...
        Returns:
            Scaled, directional error between projected and detected points.
        """
        # Simulate the 3D trajectory with the current velocity parameters
        trajectory_3d = self.simulate_trajectory(velocity_params, t_values)
        trajectory_3d = self.resample_trajectory(trajectory_3d, len(detected_2d))

        # Project the 3D points to 2D
        projected_2d = np.array([self.project_to_image(point) for point in trajectory_3d])
        ...
        # Apply weights that prioritize early frames
        weights = np.linspace(1, 0.2, len(error))  # Give more weight to earlier points
        weighted_error = error * weights[:, np.newaxis]

        error_flattened = np.sqrt(np.mean(weighted_error ** 2))

        # Debugging: Print the current error to observe optimization progress
        print(f"Velocity Params: {velocity_params}, Weighted Error Sum: {error_flattened}")

        return error_flattened
    ...
    def optimize_trajectory(self, detected_2d, t_values):
        ...
        initial_velocity_guess = np.array([-0.3, 25, 0])

        # Non-gradient optimization for initial guess
        result_non_gradient = minimize(
            self.objective_function,
            initial_velocity_guess,
            args=(detected_2d, t_values),
            method='Powell',
            bounds=[(-1, 1), (15, 45), (-10, 5)],
            options={'maxiter': 2000}
        )

        # Take the result from the non-gradient method as the new initial guess
        initial_velocity_guess = result_non_gradient.x

        # Use gradient-based optimization for fine-tuning
        ...
        optimized_velocities = result_gradient.x
        print(
            f"Final Optimized Velocities after Fine-Tuning: vx0={optimized_velocities[0]}, vy0={optimized_velocities[1]}, vz0={optimized_velocities[2]}")

        # Print optimization status and final cost
        print("Final cost after fine-tuning:", result_gradient.fun)

        return optimized_velocities

    def objective_function(self, velocity_params, detected_2d, t_values):
        """
        Objective function for optimization. Sums squared error between projected and detected points.
        """
        error = self.cost_function(velocity_params, detected_2d, t_values)
        return np.sum(error ** 2)  # Sum of squared errors for optimization


    def optimize_trajectory_stepwise(self, detected_2d, t_values):
        initial_velocity_guess = [-0.3, 25, 0]
        result_non_gradient = minimize(self.objective_function, initial_velocity_guess, args=(detected_2d, t_values),
                                       method='Powell', bounds=[(-1, 1), (15, 45), (-20, 5)], options={'maxiter': 2000})
        initial_velocity_guess = result_non_gradient.x
        result_gradient = minimize(self.objective_function, initial_velocity_guess, args=(detected_2d, t_values),
                                   method='BFGS', options={'maxiter': 2000})
        print(
            f"Final Optimized Velocities after Fine-Tuning: vx0={result_gradient.x[0]}, vy0={result_gradient.x[1]}, vz0={result_gradient.x[2]}")
        return result_gradient.x

    def run_multiple_initial_guesses(self, detected_2d, t_values):
        """
        Test multiple initial guesses for vy to find the best fit.
        """
        initial_guesses_vx = [-0.4, -0.3, -0.2, 0, 0.2, 0.3, 0.5]  # Try different forward velocities
        initial_guesses_vy = [15, 20, 25, 30, 35, 40, 45]  # Try different forward velocities
        initial_guesses_vz = [0, 5, -1, -2, -3, -5, -10]  # Try different forward velocities

        best_cost = float('inf')
        best_velocities = None

        for vy0 in initial_guesses_vy:
            initial_velocity_guess = [-0.3, vy0, 0]
            optimized_velocities = self.optimize_trajectory_stepwise(detected_2d, t_values)
            final_cost = self.objective_function(optimized_velocities, detected_2d, t_values)

            if final_cost < best_cost:
                best_cost = final_cost
                best_velocities = optimized_velocities

            print(f"Initial vy0: {vy0}, Final Cost: {final_cost}")

        print(f"Best Velocities: {best_velocities} with Cost: {best_cost}")
    ...
    def compute_t_values(self, num_detections, fps=60):
        """
        Computes evenly spaced time values based on the number of detections and frame rate.
        Args:
            num_detections: The total number of detections.
            fps: Frame rate of the video in frames per second.

        Returns:
            Array of time values corresponding to each detection.
        """

        return np.linspace(0, (num_detections - 1) * 1 / fps, num_detections)
    ...
    def run(self):
        ...
        optimized_params = self.run_multiple_initial_guesses(detected_2d, t_values)
