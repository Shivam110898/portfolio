        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <ul id="progress-bar" class="progressbar">
                    ...
                    <li class="active" id="step2" <?php if (isset($_SESSION['boxxcolour'])) { ?> onclick='return showSize();'
                        <?php } ?>>Size</li>
            ...
            <div class="col-md-12">
                ...
                <div id="hamperSize" class="panels" data-aos="fade-left">
                    <button class="accordion">Choose your Boxx size</button>
                    <div class="div-inline-list">
                        <?php  
                            $size_product_category = $db->query('SELECT id,name,price,SKU,image,quantity FROM products where is_deleted = ? AND status = ? AND quantity > 0 ORDER BY visibility_order asc', 0,1)->fetchAll();
        
                            foreach($size_product_category as $row) {
                                $pc = $db->query('SELECT product_id FROM products_to_category where category_id = ? AND product_id = ?',3, $row['id'])->fetchArray();
                                if ($row['id'] == $pc["product_id"]){ 
                                    $getDimensions = $db->query('SELECT property_value FROM product_properties where product_id = ? AND property_name = ?', $row['id'], 'Size')->fetchArray();
                                    $getColour = $db->query('SELECT property_value FROM product_properties where product_id = ? AND property_name = ?', $row['id'], 'Colour')->fetchArray();
            
                                    foreach($_SESSION['boxxcolour'] as $id => $q){
                                        $getSetHamperColour = $db->query('SELECT name, image FROM products where id = ? ', $id)->fetchArray();
                                        $image = $getSetHamperColour["image"];
                                        $image_src = "/uploadedimgs/" . $image;
                                        
                                        if($getSetHamperColour['name'] == $getColour['property_value']){
                                            ?>
                                            <div class="product-container hamperBasketCard">
                                                <input id="<?php echo $row["id"]; ?>" type="radio" name="boxxSizeRadio" hidden>
                                                <label for="<?php echo $row["id"]; ?>" class="clickable"><span
                                                        class="checked-box">&#10004;</span></label>
                                                <img src="<?php echo $image_src; ?>" alt="<?php echo $row["name"]; ?>">
                                                <p class="name"><?php echo $row['name']; ?></p>
                                                <p class="property"><?php echo $getDimensions['property_value']; ?></p>
