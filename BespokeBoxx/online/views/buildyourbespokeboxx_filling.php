    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                <ul id="progress-bar" class="progressbar">
                    ...
                    <li class="active" id="step2" <?php if (isset($_SESSION['boxxcolour'])) { ?> onclick='return showSize();'
                        <?php } ?>>Size</li>
                    <li class="active" id="step3" <?php if (isset($_SESSION["boxxsize"])) { ?> onclick='return showFilling();' <?php } ?>>
                        Filling</li>
            ...
            <div class="col-md-12">
                ...
                <div id="hamperFilling" class="panels" data-aos="fade-left">
                    <button class="accordion">Choose up to 2 FREE fillings</button>
                    <div class="div-inline-list">

                        <?php 
                    $filling_product_category = $db->query('SELECT product_id FROM products_to_category where category_id = ?', 10)->fetchAll();
                    foreach($filling_product_category as $pc) {

                        $row = $db->query('SELECT id,name,price,SKU,image,quantity FROM products where id = ? AND is_deleted = ? AND status = ? ORDER BY created_at asc',$pc['product_id'], '0','1')->fetchArray();
                     
                        $image = $row['image'];
                        $image_src = "/uploadedimgs/" . $image;
                        ?>
                        <!-- create the product container the user sees -->
                        <div class="product-container hamperFillingCard">
                            <input id="<?php echo $row["id"]; ?>" type="checkbox" name="boxxFillingCheckbox" hidden>
